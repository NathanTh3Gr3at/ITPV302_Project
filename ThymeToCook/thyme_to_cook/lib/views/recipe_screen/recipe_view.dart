...
class _RecipeViewState extends State<RecipeView> with SingleTickerProviderStateMixin {
  ...
  List<RecipeIngredient> _adjustIngredients() {
    return widget.recipe.recipeIngredients.map((ingredient) {
      double adjustedQuantity = (ingredient.quantity ?? 0) * (recipeServings / initialServings);
  ...
  double _convertUnits(double quantity, String unitBeforeConversion, String unitAfterConversion) {
    // Conversion logic
    if (unitBeforeConversion == "cup" && unitAfterConversion == "ml") {
      ...
      return quantity / 28.35; 
    } else if ((unitBeforeConversion == "lb" || unitBeforeConversion == "pound" || unitBeforeConversion == "pounds") && unitAfterConversion == "kg") {
      return quantity * 0.453592; 
    } else if ((unitBeforeConversion == "kg") && (unitAfterConversion == "lb" || unitAfterConversion == "pound" || unitAfterConversion == "pounds")) {
      return quantity / 0.453592;
    } else if ((unitBeforeConversion == "qt" || unitBeforeConversion == "quart" || unitBeforeConversion == "qz") && unitAfterConversion == "l") {
      return quantity * 0.946353; 
    } else if ((unitBeforeConversion == "l") && (unitAfterConversion == "qt" || unitAfterConversion == "quart" || unitAfterConversion == "qz")) {
      return quantity / 0.946353; 
      ...
      children: [
        ...
        decoration: BoxDecoration(
          image: DecorationImage(
                  image: NetworkImage(widget.recipe.imageSrc ?? widget.recipe.imageUrl ?? ""),
                  fit: BoxFit.cover,
                ),
        ),
      ),
...
Column instructions() {
      ...
      Expanded(
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            RecipeInstructions instruction = widget.recipe.recipeInstructions[index];
            return Padding(
 ...
 Column ingredients() {
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return Padding(
                ...
                child: Row(
                  children: [
                    if (formattedQuantity.isNotEmpty && formattedQuantity != "0")
                      Text(
                        formattedQuantity,
                        style: const TextStyle(
                          fontWeight: FontWeight.bold,
  ...
  Column overview() {
    ...
    return const Column (
      children: [
        Text("This is the nutritional information")
      ]
    );
  }
